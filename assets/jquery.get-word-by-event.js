...
(function ($) {
	...
	function expandRangeToWord(range) {
		...
		for(startOffset = range.startOffset; startOffset > 0; startOffset--) {
			if (text[startOffset].match(/[^\w_\.]/)) {
				startOffset++;
				break;
			}
		}
		range.setStart(range.startContainer, startOffset);
		for(endOffset = range.endOffset; endOffset < text.length; endOffset++) {
			if (text[endOffset].match(/[^\w_\.]/)) {
				endOffset--;
				break;
			}
		}
